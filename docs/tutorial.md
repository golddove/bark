...
| actions | 自定义通知操作按钮。传入 JSON 数组，每个操作包含：`title`（必需），`id`（可选标识符），`url`（可选链接），以及选项 `destructive`、`authenticationRequired`、`foreground`（均为布尔值）。最多支持 4 个自定义操作。示例：`[{"title":"打开","url":"https://example.com"},{"title":"删除","destructive":true}]` |
